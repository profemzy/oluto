  ...
  const Section = ({ title, accounts, total, colorClass }: {
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className="divide-y divide-gray-100">
        {accounts.length === 0 ? (
          <div className="px-6 py-4 text-sm text-gray-500">No accounts</div>
        ) : (
          accounts.map((a) => (
            <div key={a.account_id} className="flex justify-between px-6 py-3">
              <span className="text-sm text-gray-900">{a.name}</span>
              <span className="text-sm font-bold text-gray-700">{formatCurrency(a.net_balance)}</span>
            </div>
          ))
        )}
      </div>
        ...
        {report && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex justify-between items-center">
              <span className="text-lg font-bold text-gray-900">Accounting Equation</span>
              <span className={`inline-flex items-center rounded-full px-4 py-1.5 text-sm font-bold ${report.is_balanced ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}>
