...
export default function ReconciliationPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const [sum, sugg, unrec] = await Promise.all([
        api.getReconciliationSummary(user.business_id),
        api.getReconciliationSuggestions(user.business_id),
        api.getUnreconciledTransactions(user.business_id, 50),
      ]);
      setSummary(sum);
      setSuggestions(sugg);
  ...
  const handleConfirm = async (suggestion: ReconciliationSuggestion) => {
    ...
    try {
      await api.confirmMatch(user.business_id, {
  ...
  const handleReject = async (suggestion: ReconciliationSuggestion) => {
    ...
    setActionLoading(suggestion.suggestion_id);
    try {
      await api.rejectMatch(user.business_id, {
  ...
  const handleAutoReconcile = async () => {
    ...
    try {
      const result = await api.autoReconcile(user.business_id, 0.9);
      ...
      alert(
        result.matched_count > 0
          ...
          : "No high-confidence matches found to auto-reconcile"
      );
    } catch (err) {
      alert(err instanceof Error ? err.message : "Auto-reconcile failed");
    } finally {
  ...
  // Filter unreconciled transactions that don't already have a suggestion
  const suggestedTxnIds = new Set(suggestions.map((s) => s.transaction.id));
  const unreconciledOnly = unreconciled.filter(
    (t) => !suggestedTxnIds.has(t.id)
  );
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 relative z-10">
        ...
        <div className="mb-8 flex items-center justify-between">
          ...
          <button
            ...
            disabled={autoReconciling || suggestions.length === 0}
        ...
        {suggestions.length === 0 ? (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-12 text-center mb-8">
            ...
            <p className="text-sm text-gray-500">
              Import bank transactions and record payments to see matching suggestions here.
            </p>
          </div>
        ) : (
          <div className="space-y-4 mb-8">
            {suggestions.map((suggestion) => {
              ...
              const isProcessing = actionLoading === suggestion.suggestion_id;
        ...
        {suggestions.length === 0 && unreconciledOnly.length === 0 && summary && summary.total_transactions > 0 && (
...
function SummaryCard({
  ...
  return (
    <div className={`rounded-2xl bg-gradient-to-br ${colorMap[color]} p-5 shadow-sm border border-gray-100`}>
